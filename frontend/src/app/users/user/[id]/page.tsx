    ...
    if(user){
      if(user.success === false){
        ...
        return <div className="text-4xl w-screen h-[80vh] space-y-5 flex justify-center items-center text-primary flex-col">
          ...
          <button onClick={()=>router.push('/')} className="text-primary border-primary border px-3 py-2 text-2xl rounded hover:bg-primary hover:text-white cursor-pointer transition-all duration-200">
            Back To Home
          </button>
        </div> 
      }
    ...
    <div>
     <div className="bg-white text-gray-900 min-h-screen p-4 md:p-8">
      ...
      <div className="relative w-full h-48 md:h-64 lg:h-80 mb-6 rounded-lg overflow-hidden">
        <Image
          src={user.coverImage === 'No Cover Image' ? assets.noCoverImage : user.coverImage}
          alt="user banner"
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold text-white tracking-wider drop-shadow-2xl">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
          ...
          <div className="flex-shrink-0">
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-gray-900 flex items-center justify-center overflow-hidden border-4 border-gray-200">
              <div className="text-white text-5xl md:text-6xl font-bold">
                <Image unoptimized={true} src={user.avatar} width={160} height={160} alt="avatar"/>
              </div>
            </div>
          </div>
          ...
          <div className="flex-grow">
            <div className="flex items-center gap-2 mb-2">
              <h2 className="text-2xl md:text-3xl font-semibold">{user.fullName.firstname} {user.fullName.lastname}</h2>
        ...
        <div className="mt-8 border-b border-gray-800">
          <nav className="flex gap-6 overflow-x-auto">
            <button className="px-4 py-3 text-sm font-medium border-b-2 border-white whitespace-nowrap">
              Home
            </button>
            <button className="px-4 py-3 text-sm font-medium text-gray-400 hover:text-white border-b-2 border-transparent hover:border-gray-600 whitespace-nowrap">
              Videos
            </button>
            <button className="px-4 py-3 text-sm font-medium text-gray-400 hover:text-white border-b-2 border-transparent hover:border-gray-600 whitespace-nowrap">
              Shorts
